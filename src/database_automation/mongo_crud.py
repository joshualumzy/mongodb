...
class mongo_operation:
    ...
    def insert_record(self, record: dict, collection_name: str) -> Any:
        ...
        if type(record) == list:
            for data in record:
                if type(data) != dict:
                    raise TypeError(
                        "record must be a dictionary. Example is given in the docstring of this function."
                    )
    ...
    def update(self, where_condition: dict, update_query: dict, update_all_data=False):
        ...
        if update_all_data:
            self.__connect_collection.update_many(
                where_condition, {"$set": update_query}
            )
        else:
            self.__connect_collection.update_one(
                where_condition, {"$set": update_query}
            )
